...
contract Testx is Test {
    ...
    //     vm.startPrank(user1);
    //     // user1이 lending에 USDC 100000000 ether를 예금함
    //     usdc.approve(address(lending), type(uint256).max);
    ...
    //     // user2가 lending에 ETH 100000000 ether를 예금함
    ...
    //     // USER1이 Lending에 USDC 100000000 ether 예치
    //     supplyUSDCDepositUser1();

    //     // USER2가 Lending에 ETH 1 ether 예치
    //     supplySmallEtherDepositUser2();

    //     dreamOracle.setPrice(address(0x0), 1339 ether);

    //     // 1: 1339 비율
    //     // 1000 ETH: 1339000 USDC

    //     vm.startPrank(user2);
    //     {
    //         (bool success,) = address(lending).call(
    //             // USER2가 Lending에 USDC 1000 ether 요청 -> 실패
    ...
    //     // USER2가 LENDING에 ETH 1 ether를 예치함
    //     supplySmallEtherDepositUser2();

    //     // 1: 99999999999 비율
    //     // 1000 ETH: 99999999999000 USDC
    //     dreamOracle.setPrice(address(0x0), 99999999999 ether);

    //     vm.startPrank(user2);
    //     {
    //         (bool success,) = address(lending).call(
    //             // USER2가 LENDING을 통해 USDC 1000 ether 대출 요청 -> 실패
    //             abi.encodeWithSelector(DreamAcademyLending.borrow.selector, address(usdc), 1000 ether)
    //         );
    //         assertFalse(success);
    //         assertTrue(usdc.balanceOf(user2) == 0 ether);
    ...
    //         vm.roll(block.number + 1); // 이자고려

    //         (success,) = address(lending).call(
    //             abi.encodeWithSelector(DreamAcademyLending.repay.selector, address(usdc), 1000 ether)    // 1000USDC REPAY
    //         );
    //         assertTrue(success);

    //         (success,) = address(lending).call(
    //             abi.encodeWithSelector(DreamAcademyLending.borrow.selector, address(usdc), 1000 ether)   // 1000USDC BORROW
    //         );
    //         assertFalse(success);

    //         (success,) = address(lending).call(
    //             abi.encodeWithSelector(DreamAcademyLending.borrow.selector, address(usdc), 999 ether)    // 999USDC BORROW
    //         );
    //         assertTrue(success);
    //     }
    //     vm.stopPrank();
    ...
    function testCalc1() external {
        // uint256 rate_n = 100000013881950033933776467165444046258926391601562;
        // uint256 rate_d = 100000000000000000000000000000000000000000000000000;
        lending.calculate(1 ether, 1001, 1000, 6);
    }
    ...
    //     // USER3가 USDC 30000000 ether를 예금
    ...
    //     vm.roll(block.number + (86400 * 1000 / 12));          // 7,200,000
    //     vm.prank(user3);
    //     assertTrue(lending.getAccruedSupplyAmount(address(usdc)) / 1e18 == 30000792);   // 1.0000264   -> 792 어떻게 나오냐

    //     vm.roll(block.number + (86400 * 500 / 12));           // 3,600,000
    //     vm.prank(user3);
    //     assertTrue(lending.getAccruedSupplyAmount(address(usdc)) / 1e18 == 30001605);   // 1.0000535
    ...
    //     dreamOracle.setPrice(address(0x0), (4000 * 66 / 100) * 1e18); // drop price to 66% - 오라클의 가격변동
    //     usdc.transfer(user3, 3000 ether);
